...
public class ParseExtDoc {
	...
	private static void parseHtmlToXhtml(String path) throws Exception {
		File xhtmlPath = new File(xhtmlpPath);
		if (!xhtmlPath.exists()) {
			xhtmlPath.mkdir();
		}
	...
	private static Config getConfig(NodeList tds) {
		...
		if (l < 3)
			...
			StringWriter writer = new StringWriter();
			try {
				writeNode(msec.item(2), writer);
				config.setCommonts(writer.toString());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private static NodeList getNodes(XPath xpath, String exp, Document doc) throws Exception {
		xpath.setNamespaceContext(new NamespaceContext() {
			public Iterator getPrefixes(String namespaceURI) {
				...
				if (namespaceURI.equals("http://www.extjs.com"))
					map.add("ext");
			...
			public String getPrefix(String namespaceURI) {
				if ("http://www.extjs.com".equals(namespaceURI))
					return "ext";
	...
	private static void writeElement(Element el, Writer writer) throws Exception {
		 ...
		 * tformer.setOutputProperty("{http://www.extjs.com}ext", "1");
		 * DOMSource source = new DOMSource(el); Result d = new
		 * StreamResult(writer); tformer.transform(source, d);
		 */
	}

	private static void writeNode(Node node, Writer writer) throws Exception {
		java.util.Properties xmlProps = OutputPropertiesFactory.getDefaultMethodProperties("xml");
		xmlProps.setProperty("indent", "yes");
		xmlProps.setProperty("standalone", "no");
		Serializer serializer = SerializerFactory.getSerializer(xmlProps);
