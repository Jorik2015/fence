...
<div class="description">
    <p>This is a layout that enables anchoring of contained
    elements relative to the container's dimensions. If the
    container is resized, all anchored items are automatically
    rerendered according to their 
    <b>
      <tt>
    ...
    keyword.</p>
    <p>AnchorLayout does not have any direct config options (other
    than inherited ones). By default, AnchorLayout will calculate
    anchor measurements based on the size of the container itself.
    However, the container using the AnchorLayout can supply an
    anchoring-specific config property of 
    <b>anchorSize</b>. If anchorSize is specifed, the layout will
    use it as a virtual container for the purposes of calculating
    anchor measurements based on it instead, allowing the container
    to be sized independently of the anchoring logic if necessary.
    For example:</p>
<pre>
<code>
<b>var</b> viewport = 
...
@FacesComponent(value = "Ext.layout.AnchorLayout")
public class AnchorLayout extends ContainerLayout {
	public static final String COMPONENT_FAMILY = "Ext.layout.AnchorLayout";

	/**
	 * <p>
	 * Create a new {@link Ext.layout.AnchorLayout} instance with default property values.
	 * </p>
	 */
	public AnchorLayout() {
		super();
