...
<div class="description">
    ...
    table. The total number of columns can be specified, and
    rowspan and colspan can be used to create complex layouts
    within the table. This class is intended to be extended or
    created via the layout:'table' 
    ...
    keyword.</p>
    <p>Note that when creating a layout via config, the
    layout-specific config properties must be passed in via the 
    <a ext:cls="Ext.Container" ext:member="layoutConfig" href="output/Ext.Container.html#Ext.Container-layoutConfig">Ext.Container.layoutConfig</a> object
    which will then be applied internally to the layout. In the
    case of TableLayout, the only valid layout config property is 
    <a ext:cls="Ext.layout.TableLayout" ext:member="columns" href="output/Ext.layout.TableLayout.html#Ext.layout.TableLayout-columns">columns</a>. However, the
    items added to a TableLayout can supply the following
    table-specific config properties:</p>
    <ul>
      <li>
      <b>rowspan</b> Applied to the table cell containing the
      item.</li>
      <li>
      <b>colspan</b> Applied to the table cell containing the
      item.</li>
      <li>
      <b>cellId</b> An id applied to the table cell containing the
      item.</li>
      <li>
      <b>cellCls</b> A CSS class name added to the table cell
      containing the item.</li>
    </ul>
    <p>The basic concept of building up a TableLayout is
    conceptually very similar to building up a standard HTML table.
    You simply add each panel (or "cell") that you want to include
    along with any span attributes specified as the special config
    properties of rowspan and colspan which work exactly like their
    HTML counterparts. Rather than explicitly creating and nesting
    rows and columns as you would in HTML, you simply specify the
    total column count in the layoutConfig and start adding panels
    in their natural order from left to right, top to bottom. The
    layout will automatically figure out, based on the column
    count, rowspans and colspans, how to position each panel within
    the table. Just like with HTML tables, your rowspans and
    colspans must add up correctly in your overall layout or you'll
    end up with missing and/or extra cells! Example usage:</p>
<pre>
<code>
<i>// This code will generate a layout table that is 3 columns by 2
rows</i>
<i>// 
<b>with</b> some spanning included.  The basic layout will be:</i>
<i>// +--------+-----------------+</i>
<i>// |   A    |   B             |</i>
<i>// |        |--------+--------|</i>
<i>// |        |   C    |   D    |</i>
...
<i>// The total column count must be specified here</i>
    ...
    items: [{
        html: 
<em>'&lt;p&gt;Cell A content&lt;/p&gt;'</em>,
        rowspan: 2
    },{
        html: 
<em>'&lt;p&gt;Cell B content&lt;/p&gt;'</em>,
        colspan: 2
    },{
        html: 
...
@FacesComponent(value = "Ext.layout.TableLayout")
public class TableLayout extends ContainerLayout {
	public static final String COMPONENT_FAMILY = "Ext.layout.TableLayout";

	/**
	 * <p>
	 * Create a new {@link Ext.layout.TableLayout} instance with default property values.
	 * </p>
	 */
	public TableLayout() {
		super();
