...
public class MessageFactory {
	...
	public static FacesMessage getMessage(Locale locale, String messageId, Object... params) {
		...
		if (summary == null) {
			// see if we have a summary in the app provided bundle
			bundle = ResourceBundle.getBundle(MOJARRA_RESOURCE_BASENAME, locale, getCurrentLoader(bundleName));
			if (null == bundle) {
				throw new NullPointerException();
			}
			// see if we have a hit
	 ...
	 * substitution parameter in the message. For example: <code>#{bundle.userLabel}</code> "bundle" may not be
	 * available until the page is rendered. The "late" binding evaluation in <code>getSummary</code> and
