...
import com.abner.fence.utils.ClassUtils;
...
public class JsonLevelFilter implements PropertyFilter {
	...
	public boolean apply(Object source, String name, Object value) {
		...
		String methodName = "get" + Strings.upperFirstChar(name);
		...
		if (sourceLevel < 0) {
			...
			if (level < 2) {
				...
				if (!clazz.isPrimitive() && !clazz.getName().equals("java.lang.String")
						&& !ClassUtils.isRepresentsPrimitive(clazz) && !ClassUtils.isDate(clazz)) {
					// 否则缓存，将在下一步中不处理。
	...
	private boolean isSimple(Class<?> clazz) {
		return clazz.isPrimitive() || clazz.getName().equals("java.lang.String")
				|| ClassUtils.isRepresentsPrimitive(clazz) || ClassUtils.isDate(clazz);
	}
