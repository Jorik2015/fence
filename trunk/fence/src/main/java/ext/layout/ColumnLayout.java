 ...
 * 
    ...
    keyword.
    ColumnLayout does not have any direct config options (other
    ...
    auto).
    The width property is always evaluated as pixels, and must
    be a number greater than or equal to 1. The columnWidth
    property is always evaluated as a percentage, and must be a
    decimal value greater than 0 and less than 1 (e.g., .25).
    The basic rules for specifying column widths are pretty
    ...
    on the total 
    remaining container width. In other words, percentage
