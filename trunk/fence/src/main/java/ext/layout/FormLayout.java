 ...
 * 
  ...
  forms. It is responsible for
  rendering the labels of 
  Fields.
  This layout manager is used when a Container is configured
  with the 
  layout:'form' 
  layout config
  ...
  A 
  Container 
  using the FormLayout layout manager (e.g. 
  Ext.form.FormPanel or specifying
  
  layout:'form') can also accept the following
  ...
  Any Component (including Fields) managed by FormLayout accepts
  the following as a config option:
  ...
  Any Component managed by FormLayout may be rendered as a form
  ...
  . Components configured in this way may be configured with
...
// Required 
if showing validation messages
Ext.QuickTips.init();

// While you can create a basic Panel 
with layout:
'form', practically
...
labelSeparator: 
':' 
// override labelSeparator layout config
        },{
            fieldLabel: 
'Last Name',
            name: 
...
'textarea',
            hideLabel: true,     
// override hideLabels layout config
            name: 
'msg',
            anchor: 
'100% -53'
...
labelSeparator: 
'~' 
// superseded by assignment below
    },
    
// config options applicable to container when layout=
'form':
    hideLabels: false,
    labelAlign: 
'left',   
...
labelSeparator: 
'>>', 
// takes precedence over layoutConfig value
    labelWidth: 65,       
// defaults to 100
    labelPad: 8           
