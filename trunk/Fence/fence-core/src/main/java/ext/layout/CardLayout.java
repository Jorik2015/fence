...
<div class="description">
    <p>This layout manages multiple child Components, each fitted
    to the Container, where only a single child Component can be
    visible at any given time. This layout style is most commonly
    used for wizards, tab implementations, etc. This class is
    intended to be extended or created via the layout:'card' 
    ...
    keyword.</p>
    <p>The CardLayout's focal method is 
    <a ext:cls="Ext.layout.CardLayout" ext:member="setActiveItem" href="output/Ext.layout.CardLayout.html#Ext.layout.CardLayout-setActiveItem">setActiveItem</a>. Since only
    one panel is displayed at a time, the only way to move from one
    Component to the next is by calling setActiveItem, passing the
    id or index of the next panel to display. The layout itself
    does not provide a user interface for handling this navigation,
    so that functionality must be provided by the developer.</p>
    <p>In the following example, a simplistic wizard setup is
    demonstrated. A button bar is added to the footer of the
    containing panel to provide navigation buttons. The buttons
    will be handled by a common navigation routine -- for this
    example, the implementation of that routine has been ommitted
    since it can be any type of custom logic. Note that other uses
    of a CardLayout (like a tab control) would require a completely
    different implementation. For serious implementations, a better
    approach would be to extend CardLayout to provide the custom
    functionality needed. Example usage:</p>
<pre>
<code>
<b>var</b> navHandler = 
<b>function</b>(direction){
    
<i>// This routine could contain business logic required to manage
the navigation steps.</i>
    
<i>// It would call setActiveItem as needed, manage navigation
button state, handle any</i>
    
<i>// branching logic that might be required, handle alternate
actions like cancellation</i>
    
<i>// or finalization, etc.  A complete wizard implementation could
get pretty</i>
    
<i>// sophisticated depending on the complexity required, and
should probably be</i>
    
<i>// done as a subclass of CardLayout 
<b>in</b> a real-world implementation.</i>
};
...
<em>'card'</em>,
    activeItem: 0, 
<i>// make sure the active item is set on the container config!</i>
    bodyStyle: 
<em>'padding:15px'</em>,
    defaults: {
...
<i>// just an example of one possible navigation scheme, using
buttons</i>
    bbar: [
        {
            id: 
...
<em>'-&gt;'</em>, 
<i>// greedy spacer so that the buttons are aligned to each
side</i>
        {
            id: 
<em>'move-next'</em>,
...
<i>// the panels (or 
<em>"cards"</em>) within the layout</i>
    items: [{
        id: 
<em>'card-0'</em>,
        html: 
<em>'&lt;h1&gt;Welcome to the Wizard!&lt;/h1&gt;&lt;p&gt;Step 1 of
3&lt;/p&gt;'</em>
    },{
        id: 
<em>'card-1'</em>,
        html: 
<em>'&lt;p&gt;Step 2 of 3&lt;/p&gt;'</em>
    },{
        id: 
<em>'card-2'</em>,
        html: 
<em>'&lt;h1&gt;Congratulations!&lt;/h1&gt;&lt;p&gt;Step 3 of 3 -
Complete&lt;/p&gt;'</em>
    }]
});</code>
    </pre>
...
@FacesComponent(value = "Ext.layout.CardLayout")
public class CardLayout extends FitLayout {
	public static final String COMPONENT_FAMILY = "Ext.layout.CardLayout";

	/**
	 * <p>
	 * Create a new {@link Ext.layout.CardLayout} instance with default property values.
	 * </p>
	 */
	public CardLayout() {
		super();
