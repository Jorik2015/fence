...
public class ExtUtils {
	...
	public static void applyConfig(ExtComponent toComp, Object config) {
	...
	public static Object applyBean(Object bean, Object config) {
		...
		for (Iterator<?> keys = configJson.keys(); keys.hasNext();) {
			...
			try {
				...
				if (clazz.isPrimitive() || ClassUtils.isRepresentsPrimitive(clazz) || clazz.equals(String.class)) {
					...
					if (Strings.isEmpty(value)) {
						...
						if (value instanceof String) {
							...
							SimpleDateFormat df = new SimpleDateFormat(DateJsonValueProcessor.DEFAULT_PATTERN);
							...
							if (clazz.equals(java.sql.Date.class)) {
								PropertyUtils.setProperty(bean, key, new java.sql.Date(date.getTime()));
							} else {
					...
					PropertyUtils.setProperty(bean, (String) key, origPropValue);
				}
			} catch (IllegalAccessException e) {
				throw new RuntimeException("The store bean can write with some property.");
