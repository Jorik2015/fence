// from http://joekuan.wordpress.com/category/web/highcharts/
// http://github.com/JoeKuan/Highcharts-Ext-JS-Adapter 
 ...
 * @xtype highchart
 */
Ext.ux.HighChart = Ext
		.extend(
				Ext.BoxComponent,
				{

					/**
					 * @cfg {Object} defaultSerieType Sets styles for this
					 *      chart. This contains default styling, so modifying
					 *      this property will <b>override</b> the built in
					 *      styles of the chart. Use {@link #extraStyle} to add
					 *      customizations to the default styling.
					 */
					defaultSerieType : null,

					/**
					 * @cfg {Boolean} resizable True to allow resizing, false to
					...
					/**
					 * @cfg {Object} loadMask An {@link Ext.LoadMask} config or
					 *      true to mask the chart while loading. Defaults to
					 *      false.
					 */
					loadMask : false,

					/**
					 * @cfg {Boolean} animShift Special type of animation for
					 *      non pie chart True to animate and shift the
					 *      line/spline/bar chart when new data arrive This
					 *      implies extra checking of new data arrived in xField
					 */
					animShift : false,
					...
					addSeries : function(series, append) {
						...
						// Add empty data to the serie or just leave it normal.
						// Bug in HighCharts?
						for ( var i = 0; i < series.length; i++) {
							var serie = series[i];
							if (!serie.serieCls) {
								if (serie.type != null
										|| this.defaultSerieType != null) {
									cls = Ext.ux.HighChart.Series
											.get(serie.type != null ? serie.type
						...
						if (this.chart) {
							if (!append) {
								...
								this.chartConfig.series = this.chartConfig.series ? this.chartConfig.series
										.concat(c)
										: c;
					...
					initEvents : function() {
						if (this.loadMask) {
							this.loadMask = new Ext.LoadMask(this.el, Ext
									.apply({
										store : this.store
									}, this.loadMask));
						}
					...
					draw : function() {
						...
						if (this.chart && this.rendered) {
							...
							// The only way to get initial animation working is
							// to populate the
							// data into the config object first from loading a
							// data store
							// Only allow initial animation if none of series
							// has defined animation: false
							var initAnim = true;
							for ( var s = 0; s < this.series.length; s++) {
								// Look for the type and look into associated
								// animation config in chartConfig
								var ctype = this.series[s].type;
								if (this.chartConfig.plotOptions) {
									var c_anim = (this.chartConfig.plotOptions[ctype] !== undefined && this.chartConfig.plotOptions[ctype].animation === false) ? false
											: true;
									if (!c_anim) {
										initAnim = false;
										break;
							...
							if (initAnim)
								initAnim = (this.chartConfig.plotOptions
										&& this.chartConfig.plotOptions.series !== undefined && this.chartConfig.plotOptions.series.animation === false) ? false
										: true;
											...
											callback : function(records,
												...
												for ( var i = 0; i < this.chartConfig.series.length; i++) {
													...
													if (this.series[i].dataIndex) {
														...
														xField = this.xField;
													} else if (this.series[i].type == 'pie'
															&& this.series[i].dataField) {
					...
					bindComponent : function(bind) {
						/**
						 * Make the chart update the positions positions are
						 * based on the window object and not on the owner
						 * object.
						 */
					...
					/**
					 * Changes the data store bound to this chart and refreshes
					 * it.
					 * 
					 * @param {Store}
					...
					bindStore : function(store, initial) {
						...
						if (!initial && this.store) {
							if (store !== this.store && this.store.autoDestroy) {
								...
								this.store.un("datachanged", this.onDataChange,
										this);
					...
					refresh : function() {
						if (this.store && this.chart) {
							...
							// So we need to have all columns that we use in
							// line.
							...
							var updateAnim = (this.chartConfig.animation === false) ? false
									: (this.chartConfig.animation === undefined ? true
											: this.chartConfig.animation);
							// Update the series
							for (i = 0; i < seriesCount; i++) {

								if (this.series[i].useTotals) {
									this.chart.series[i].setData(this.series[i]
											.getTotals());
								} else if (this.chart.series[i].data.length
										&& updateAnim && !this.animShift) {

									// Align the data between chart series and
									// store and update
									// if different
									...
									if (chartSz > storeSz) {
										for (x = storeSz - 1; x < chartSz; x++) {
											...
											this.chart.series[i].data[lastIdx]
													.remove(false, updateAnim);
										}
									} else if (chartSz < storeSz) {
										for (x = chartSz; x < storeSz; x++)
											this.chart.series[i].addPoint([ 0,
													0 ], false, false,
													updateAnim);
									}
									// Update the y values, x values will be by
									// updatexAxisData if xField defined
									for (x = 0; x < storeSz; x++) {
										this.chart.series[i].data[x].update(
												this.series[i]
														.getData(items[x]),
												true, updateAnim);
									}
									...
									// Compare the last data in chart's series
									// data to the store item
									// Then add the new data points to the
									// chart's series
									// All possible scenarios:
									// i) last data in chart's series not found
									// in the the store data
									// - use setData to all the new data
									// ii) last data in chart's series is the
									// same as last data in the store data
									// - no need to update
									// iii) last data in the middle of somewhere
									// in store data
									...
									for ( var x = items.length - 1; x >= 0; x--) {
										...
										if (x_val == lastChartData) {
											// In the same last position,
											// nothing to update
											if (x == items.length - 1) {
												// alert('nothing to update');
												return;
											}
											// Somewhere in the middle, all the
											// new data points
											if (x >= 0) {
									...
									// All new data
									if (x < 0) {
										// alert('last chart data ' +
										// lastChartData + ', last store data '
										// +
										// data[i][items.length-1].data[this.xField]);
										this.chart.series[i].setData(data[i],
												(i == (seriesCount - 1))); // true
																			// ==
																			// redraw.
									}
								} else {
						...
						if (this.chart) {
							for ( var i = 0; i < this.chart.series.length; i++) {
								...
								var point = this.series[i].getData(record,
										index);
								if (this.series[i].type == 'pie'
										&& this.series[i].useTotals) {
...
Ext.ux.HighChart.PieSerie = Ext.extend(Ext.ux.HighChart.Serie,
		{
			...
			constructor : function(config) {
				Ext.ux.HighChart.PieSerie.superclass.constructor.apply(this,
						arguments);
				if (this.useTotals) {
					this.columnData = {};
			...
			update : function(record) {
				for ( var i = 0; i < this.columns.length; i++) {
					...
					if (record.modified[c])
						this.columnData[c] = this.columnData[c]
								+ record.data[c] - record.modified[c];
				}
			},
			...
			getData : function(record, index) {
				...
				return [ record.data[this.categorieField],
						record.data[this.dataField] ];
			},
