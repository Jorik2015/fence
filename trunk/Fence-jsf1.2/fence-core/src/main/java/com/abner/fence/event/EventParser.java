 ...
 * 
 * <p>
 * Convenience methods for extracting <code>FacesEvent</code> related
...
public class EventParser {
	...
	/**
	 * <p>
	 * Look in the {@link #FACES_EVENT_HEADER} and parse it according to the
	 * following syntax
	 * </p>
	 * 
	 ...
	 * 
	 * <p>
	 * Where <code>EVENT_TYPE</code> is the String <code>ValueChangeEvent</code>, <code>ActionEvent</code>, or one of the event types defined to the
	 * system in the that must be a subclass of <code>FacesEvent</code>.
	 * 
	 */
	...
	public static void queueFacesEvents(FacesContext context) {
		...
		String header = p.get(RequestUtils.FACES_EVENT_HEADER);
		String[] events = null;
		String[] params = null;
		...
		if (header != null) {
			...
			for (i = 0; i < events.length; i++) {
				...
				phaseId = Utils.getPhaseIdFromString(params[2]);
				...
				// Otherwise, if we have more than just those three pieces of
				// data...
	...
	/**
	 * <p>
	 * Given the argument <code>phaseId</code> consult the incoming request and
	 * see if there is a
	 * </p>
	 */

	public static void invokeComponentMethodCallbackForPhase(
			FacesContext context, PhaseId curPhase) {
		...
		if (null == (callback = (ComponentMethodCallback) requestMap
				.get(REQUEST_MAP_KEY))) {
			String de = headersMap.get(RequestUtils.METHOD_NAME_HEADER);
			if (de != null) {
				String[] ep = de.split(",");
				String clientId = ep[0];
				String method = (ep.length > 1) ? ep[1] : null;
				PhaseId phaseId = (ep.length > 2) ? Utils
						.getPhaseIdFromString(ep[2]) : PhaseId.RENDER_RESPONSE;

				callback = new ComponentMethodCallback(clientId, method,
						phaseId);
	...
	private static void queueFacesEvent(FacesContext context, String eventId,
			String clientId, final PhaseId phaseId, final String[] params) {
		FacesEvent result = null;
		Map<String, ConstructorWrapper> eventsMap = (Map<String, ConstructorWrapper>) context
				.getExternalContext().getApplicationMap().get(
						RequestUtils.FACES_EVENT_CONTEXT_PARAM);
			...
			context.getViewRoot().invokeOnComponent(context, clientId,
					new ContextCallback() {
						public void invokeContextCallback(
								FacesContext facesContext, UIComponent comp) {
							FacesEvent event = null;
							Class[] ctorArgClasses = eventCtor.getArgClasses();
							int len = 0;
							Object[] ctorArgs = getEventCtorArgs(facesContext,
									comp, ctorArgClasses, params);
							try {
								event = (FacesEvent) eventCtor.getConstructor()
	...
	private static Object[] getEventCtorArgs(FacesContext context,
			UIComponent comp, Class[] ctorArgClasses, String[] ctorArgValues) {
		Object[] result = null;
		Application app = context.getApplication();
		Converter converter = null;
		int argClassesLen = ctorArgClasses.length, argValuesLen = ctorArgValues.length, i = 0;
		...
		// than the argument values list...
			...
			// Iterate over both arrays and convert the elements using
			// converters
			for (i = 0; i < ctorArgClasses.length; i++) {
				...
				else if (null != (converter = app
						.createConverter(ctorArgClasses[i]))) {
					result[i] = converter.getAsObject(context, comp,
							ctorArgValues[i]);
				}
