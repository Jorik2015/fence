...
public class ExtResourcesServlet extends HttpServlet {
	...
	private void writePageScript(HttpServletRequest request,
		...
		initScript.append("}");
		builder.append(MessageFormat.format(ScriptManager.OnReadyTemplate,
				initScript));
	...
	public void init() throws ServletException {
		try {
			String path = getClass().getClassLoader().getResource(
					getClass().getName().replace('.', '/') + ".class").toURI()
					.toString();
