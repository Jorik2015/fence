...
public class ExtInputRenderer extends ExtBasicRenderer<ExtInput> {
	...
	@Override
	public void afterEncodeBegin(FacesContext context, ExtInput component)
			throws IOException {
		...
		FieldSet fieldSet = ComponentUtil.inNested(FieldSet.class, component);
		if (fieldSet != null) {
			configTo = "items";
			toComp = fieldSet;
		}
		...
		RadioGroup radGroup = ComponentUtil.inNested(RadioGroup.class,component);
		if(radGroup != null){
			configTo = "items";
			toComp = radGroup;
		}

		Column col = ComponentUtil.inNested(Column.class, component, true);
		if (col != null) {
			configTo = "editor";
			toComp = col;
		}
	...
	@Override
	public void beforeEncodeEnd(FacesContext context, ExtInput component)
			throws IOException {
	...
	@Override
	public void afterEncodeEnd(FacesContext context, ExtInput component)
			throws IOException {
		Iterator<?> messageIter = getMessageIter(context, component
		...
		if (messageIter.hasNext()) {
			...
			if (curMessage instanceof ExtFacesMessage) {
				...
				String script = emsg.getCompId() + ".markInvalid(\""
						+ emsg.getSummary() + "\");";
	...
	public Object getConvertedValue(FacesContext context,
		...
		if (null == converter && null != valueExpression) {
			Class converterType = valueExpression.getType(context
					.getELContext());
	...
	protected Object getValue(UIComponent component) {
		if (component instanceof Field) {
			...
			if (ve != null) {
				...
				ValueExpression formVe = extField
						.getFormValueExpression(FacesUtils.context());
	...
	private boolean hasStringConverter(FacesContext context) {
		if (!hasStringConverterSet) {
			hasStringConverter = (null != context.getApplication()
