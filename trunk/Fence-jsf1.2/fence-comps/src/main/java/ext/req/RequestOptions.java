...
public class RequestOptions extends Action {
	public static final String COMPONENT_TYPE = "Ext.req.RequestOptions";

	/**
	 * <p>
	 * Create a new {@link RequestOptions} instance with default property
	 * values.
	 * </p>
	 */
	 ...
	 * is called regardless of success or failure and is passed the following
	 * parameters: options : Object The parameter to the request call. success :
	 * Boolean True if the request succeeded. response : Object The
	 * XMLHttpRequest object containing the response data. See
	 * http://www.w3.org/TR/XMLHttpRequest/ for details about accessing elements
	 * of the response.
	 * */
	...
	/**
	 * Only meaningful when used with the form option. True if the form object
	 * is a file upload (will be set automatically if the form was configured
	 * with enctype "multipart/form-data"). File uploads are not performed using
	 * normal "Ajax" techniques, that is they are not performed using
	 * XMLHttpRequests. Instead the form is submitted in the standard manner
	 * with the DOM <form> element temporarily modified to have its target set
	 * to refer to a dynamically generated, hidden <iframe> which is inserted
	 ...
	 * browser to insert the text unchanged into the document body. The response
	 * text is retrieved from the document, and a fake XMLHttpRequest object is
	 * created containing a responseText property in order to conform to the
	 * requirements of event handlers and callbacks. Be aware that file upload
	 * packets are sent with the content type multipart/form and some server
	 * technologies (notably JEE) may require some custom processing in order to
	 ...
	 * for the post data. Any params will be appended to the URL.
	 */
	@ClientConfig(JsonMode.Object)
