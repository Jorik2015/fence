...
public class ExtPanelRenderer<T extends BoxComponent> extends
		ExtBasicRenderer<BoxComponent> {
	...
	private boolean canRenderContentEl(FacesContext context,
		...
		List<String> lays = Arrays.asList(new String[] { "accordion", "anchor",
				"card", "form", "menu", "table", "fit", "border", "toolbar",
				"vbox", "vbox" });
	...
	public void beforeEncodeBegin(FacesContext context, BoxComponent component)
		...
		if (component instanceof Panel) {
			...
			if (autoLoad != null && !JSONUtils.mayBeJSON(autoLoad.toString())) {
				...
				if ("true".equalsIgnoreCase(autoLoad.toString())) {
					panel.handleConfig("autoLoad", AjaxJSUtils
							.encodeUpdaterOptions(context, panel));
		...
		if (component instanceof TabPanel) {
			...
			if (NumberUtils.isNumber(activeTab)) {
				((TabPanel) component).handleConfig("activeTab",
						Ext.PREFIX_RAW_VALUE + activeTab);
			}
		}
	}

	@Override
	public void afterEncodeBegin(FacesContext context, BoxComponent component)
			throws IOException {
		...
		if (canRenderContentEl(context, component)) {
			...
			if (window == null) {
				JSUtils.RegisterBeforeClientInitScript(ScriptManager.All_HTML_CONTENT_DIV_VAR
								+ ".clear();");
				UIComponent parent = component.getParent();
				if (!RequestUtils.isAjaxRequest()) {
					if (parent instanceof TabPanel
							&& !ComponentUtil.isActiveItem(component)) {
						writer.writeAttribute("class", "x-hide-display",
								"class");
					} else {
						writer.writeAttribute("style", "visibility: hidden",
								"style");
						this.appendScriptAfter(component,
								ScriptManager.All_HTML_CONTENT_DIV_VAR
										+ ".add(\"" + contentId + "\");");
					}
				}
			}
		...
		if (parent instanceof ContainerLayout) {
			...
			if (ComponentUtil.isHaeresNatus(component)
					&& !isFloating(component)) {
				component.setRenderTo(Ext.NULL);
				ConfigHelper.configTo("items", component.getVar(), component);
			}
	...
	public void encodeChildren(FacesContext context, UIComponent component)
		...
		boolean notRenderChildren = !RequestUtils.isAjaxRequest()
				&& box instanceof Panel && this.isAutoLoad((Panel) box)
				&& !ComponentUtil.isActiveItem(box);
		if (notRenderChildren) {
			ResponseWriter writer = context.getResponseWriter();
			writer
					.write("<!-- The content is updated lazy use ajax by fence -->");
		} else {
	...
	private boolean isNeedUpdateInClient(BoxComponent component) {
		...
		if (RequestUtils.isAjaxRequest()
				&& ComponentUtil.isHaeresNatus(component)) {
			if (async.isRenderRoot(component)
					&& !RequestUtils.isUpdaterRequest()) {
				return true;
			}
			if (RequestUtils.isUpdaterRequest()
					&& async.isRenderRoot(component.getParent())) {
				return true;
			}
		}
		return false;
	...
	@Override
	public void beforeEncodeEnd(FacesContext context, BoxComponent component)
			throws IOException {
		...
		if (component instanceof MifPanel) {
			...
			if (!StringUtil.isEmpty(defaultSrc)) {
				String actionPath = FacesUtils.getViewHandler(context)
						.getActionURL(context, defaultSrc);
